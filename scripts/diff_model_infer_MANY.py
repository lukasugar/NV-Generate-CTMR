...
def set_random_seed(seed: int | None) -> int:
    ...
    Args:
        seed (int | None): Random seed. If None, a random seed is selected.
...
def prepare_tensors(args: argparse.Namespace, device: torch.device) -> tuple:
    ...
    Returns:
        tuple: Prepared top_region_index_tensor, bottom_region_index_tensor, spacing_tensor, and modality_tensor.
    """
...
def run_inference(
    ...
    """
    Run the inference to generate one synthetic image.
...
def diff_model_infer_many(env_config_path: str, model_config_path: str, model_def_path: str, num_gpus: int, num_images: int = 1) -> list[str]:
    """
    Main function to run diffusion model inference for multiple images.

    Args:
        env_config_path (str): Path to the environment configuration file.
        model_config_path (str): Path to the model configuration file.
        model_def_path (str): Path to the model definition file.
        num_gpus (int): Number of GPUs used by the distributed process group.
        num_images (int): Number of images to generate sequentially per rank.

    Returns:
        list[str]: Generated output paths for all ranks.
    """
    if num_images < 1:
    ...
    logger = setup_logging("inference_many")

    base_random_seed = args.diffusion_unet_inference.get("random_seed")
    if base_random_seed is None:
        base_random_seed = random.randint(0, 99999)
    base_random_seed = int(base_random_seed)
    logger.info(f"Using {device} of {world_size} with base random seed: {base_random_seed}")
    ...
    if local_rank == 0:
        ...
        logger.info(f"[config] base_random_seed -> {base_random_seed}.")
        logger.info(f"[config] num_images -> {num_images}.")
    ...
    for image_index in range(num_images):
        image_seed = base_random_seed + image_index + local_rank
        set_random_seed(image_seed)
        logger.info(f"Generating image {image_index + 1}/{num_images} on rank {local_rank} with random seed: {image_seed}")
        ...
        output_path = (
            f"{args.output_dir}/{output_prefix}_seed{image_seed}_image{image_index}_"
            f"size{output_size[0]:d}x{output_size[1]:d}x{output_size[2]:d}_"
            f"spacing{out_spacing[0]:.2f}x{out_spacing[1]:.2f}x{out_spacing[2]:.2f}_"
            f"{timestamp}_rank{local_rank}_modality{modality}.nii.gz"
        )
    ...
    if dist.is_available() and dist.is_initialized():
        ...
        dist.all_gather_object(gathered_paths, output_paths)
        paths = [path for rank_paths in gathered_paths for path in rank_paths]
    else:
        paths = output_paths
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Diffusion Model Inference for Multiple Images")
    ...
    parser.add_argument("-g", "--num_gpus", type=int, default=1, help="Number of GPUs to use for inference")
    parser.add_argument("--num_images", type=int, default=1, help="Number of images to generate sequentially per rank")

    args = parser.parse_args()
    diff_model_infer_many(args.env_config, args.model_config, args.model_def, args.num_gpus, args.num_images)
